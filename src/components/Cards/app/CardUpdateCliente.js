...
export const CardUpdateCliente = () => {
    const navigate = useHistory();
    const location = useLocation();
    const dataArg = location.state;

    const [errorOrOk, setErrorOrOk] = React.useState("");
    const [cliente, setCliente] = React.useState(dataArg.meta);

    const handleSubmit = async (evt) => {
        evt.preventDefault();
        setErrorOrOk("");
        try {
            await updateCustomDoc("clientes", dataArg.id, cliente);
            setCliente(dataArg.meta);
            setErrorOrOk("Cliente actualizado");
            navigate.push('/app/clients')
        } catch (error) {
            setErrorOrOk("Error al actualizar cliente");
        }
    }

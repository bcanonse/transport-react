...
const CardRegisterSucursal = () => {

    const [sucursal, setSucursal] = React.useState(STATE_INITIAL);
    ...
    const handleSubmit = async (evt) => {
        evt.preventDefault();
        sucursal.id_negocio = selectedNegocio.codigo;
        setErrorOrOk("");
        if (sucursal.nombre !== '' && sucursal.id_negocio > 0) {
            try {
                const response = await createDoc("sucursales", sucursal);

                if (response && response.id.length > 0) {
                    setSucursal(STATE_INITIAL);
                    setErrorOrOk("Sucursal creada");
                    navigate.push('/admin/sucursales/')
                }

            } catch (error) {
                setErrorOrOk("Error al crear sucursal");
            }
        }
    }
            ...
            <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
                <div className="rounded-t bg-white mb-0 px-6 py-6">
                    ...
                    <div className="text-center flex justify-between">
                        <h6 className="text-blueGray-700 text-xl font-bold">Sucursal</h6>
                    </div>
                </div>
...
export default CardRegisterSucursal;